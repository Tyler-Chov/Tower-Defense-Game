...
This file defines a Tower class used in a tower defense game.
Each Tower object has attributes such as damage, attack range, and
cooldown, and methods for attacking enemies, upgrading, and displaying
...
class Tower(ABC):
    def __init__(self, name: str, damage: int, shot_cooldown: int, price: int,
    ...
    def render(self, window):
        ...
        if self._position:
            ...
            current_image = (self._flipped_image
                             if self._is_facing_left else self._image)
            tower_surface = pygame.transform.scale(
                current_image, (self.size * 3, self.size * 3))
    ...
    def _render_range(self, window):
        ...
        range_surface = pygame.Surface(
            (self._attack_range * 2, self._attack_range * 2), pygame.SRCALPHA)
    ...
    def set_name(self, name):
        """Sets the name of the tower, raising an error if the name is
        empty."""
        if name == "":
    ...
    def upgrade_tower(self):
        """Upgrades the tower's attributes, increasing damage, range, and
        upgrade cost."""
        self._upgrade_level += 1
    ...
    def attack(self, enemies, projectiles_list):
        """Attacks the first enemy within range if the tower is not on
        cooldown."""
...
class Archer_Tower(Tower):
    ...
    def upgrade_tower(self):
        """Upgrades the tower's attributes based on the level, elimination
        requirements, and upgrade cost."""
        if self._upgrade_level == 1 and self._enemies_defeated >= 15:
            ...
            print("Requirements not met,insufficient funds, or maximum level.")
